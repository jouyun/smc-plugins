...
public class EMFPlotter extends Plotter{
	...
	public EMFPlotter(int width,int height,OutputStream os){
		this.width=width;
		this.height=height;
		shapesize=8;
		try{
			vg2=new EMFGraphics2D(os,new Dimension(width,height));
	...
	public EMFPlotter(int width,int height,String path){
		this.width=height;
		this.height=height;
		shapesize=8;
		try{
			File file=new File(path);
			vg2=new EMFGraphics2D(file,new Dimension(width,height));
			vg2.setDeviceIndependent(true);
			vg2.startExport();
		}catch(Throwable e){
			IJ.log(e.toString());
