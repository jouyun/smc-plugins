 ...
 * 
 *         Modified by Michel Boudinot (mb) 24 Aug 2006. michel.boudinot at
 *         inaf.cnrs-gif.fr - improved decoding strategy, more robust against
 *         zvi file structure diversity. - new approach to compute the number of
 *         channels. - new image stack construction from file image slices.
 * 
 *         Modified by Jay Unruh for ease of use in a library and virtual stack
 *         access
 */

public class ZVI_virtual_stack extends VirtualStack{
	boolean littleEndian=true;
	private int theC,theZ,numC,numZ,numT,width,height,size;
	...
	public ZVI_virtual_stack(String directory,String fileName){
		if(fileName==null)
			return;
		...
		FileOpener opener=new FileOpener(fi[0]);
		ImagePlus imp=opener.open(false); // is there a cleaner way?
		if(imp==null){
			IJ.showStatus("");
		...
		this.size=numC*numZ*numT;
		ImagePlus imp2=new ImagePlus(fi[0].fileName,this);
		imp2.setFileInfo(fi[0]);
		if((numC>1&&numZ>1)||(numC>1&&numT>1)||(numZ>1&&numT>1)){
			imp2.setOpenAsHyperStack(true);
			imp2.setDimensions(numC,numZ,numT);
			if(numC>1){
				imp2=new CompositeImage(imp2,CompositeImage.GRAYSCALE);
			}
		}
		imp2.show();
	}
	...
	public ImageProcessor getProcessor(int n){
		...
		FileOpener fo=new FileOpener(fi[n-1]);
	...
	private FileInfo[] getHeaderInfo(String directory,String fileName) throws IOException{
		...
		while(true){
			...
			int bitDepth=readInt(in); // doesn't always equal bytesPerPixel *
			// 8
			pos+=24;

			ZVIBlock zviBlock=new ZVIBlock(theZ,theC,theT,w,h,alwaysOne,bytesPerPixel,pixelType,bitDepth,pos);
			if(DEBUG)
		...
		for(int i=0;i<fi.length;i++){
			...
			if(ft<0){
				IJ.showMessage("ZVI Reader","Warning: unknown file type for image plane #"+(i+1));
	 ...
	 */
	private long findBlock(RandomAccessFile in,byte[] block,long start) throws IOException{
		long filePos=start;
	...
	private class ZVIBlock{
		...
		public ZVIBlock(int theZ,int theC,int theT,int width,int height,int alwaysOne,int bytesPerPixel,int pixelType,int bitDepth,long imagePos){
			...
			numChannels=1;
			// the second decision is redundant, but left there for further(?)
			// pixel types
			if((pixelType==1)|(pixelType==8)){
				numChannels=3;
			} // 1 and 8 are RGB 8-bit and 16-bit
			else if((pixelType==3)|(pixelType==4)){
				numChannels=1;
			} // 3 and 4 are GRAY 8-bit and 16-bit

			if(bytesPerPixel%numChannels!=0){
				IJ.showMessage("ZVI Reader","Warning: incompatible bytesPerPixel ("+bytesPerPixel+") and numChannels ("+numChannels+"). Assuming grayscale data.");
				numChannels=1;
			}
			bytesPerChannel=bytesPerPixel/numChannels;

			// IJ.showMessage("ZVI Reader", "numChannels: " + numChannels +
			// ", bytesPerPixel: " + bytesPerPixel + ", pixelType: " + pixelType
			// + ", bitDepth: " + bitDepth + ", imagePos: " + imagePos);
		}
