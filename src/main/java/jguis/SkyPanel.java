...
public class SkyPanel extends Panel implements ActionListener,ItemListener,MouseMotionListener,MouseListener{
	...
	public String[] names;
	/*
	 * public static int[][] code={ {0,1,1,1,0,92}, //1 {0,0,0,0,1,100}, //2
	 * {1,0,1,1,1,86}, //3 {0,0,1,1,0,88}, //4 {1,1,0,1,1,70}, //5
	 * {0,1,1,1,1,71}, //6 {0,1,1,0,0,68}, //7 {0,0,0,1,0,64}, //8
	 * {1,0,0,1,1,55}, //9 {0,0,1,0,1,60}, //10 {1,0,1,1,0,59}, //11
	 * {0,1,0,0,1,54}, //12 {1,0,0,1,0,48}, //13 {0,1,0,0,0,48}, //14
	 * {1,1,1,0,0,50}, //15 {0,1,0,1,0,43}, //16 {0,0,1,0,0,48}, //17
	 * {1,1,0,1,0,45}, //18 {1,0,1,0,0,36}, //19 {1,0,0,0,0,36}, //20
	 * {0,0,0,1,1,24}, //21 {1,1,1,0,1,32}, //22 {1,0,0,0,1,75}, //x
	 * {0,0,1,1,1,39} //y };
	 */

	public static int[][] code={{1,1,0,0,1,92}, // 1
			{1,0,0,1,0,100}, // 2
			{1,1,0,1,0,86}, // 3
			{0,0,1,0,1,88}, // 4
			{0,1,0,1,1,70}, // 5
	...
	public static Frame launch_frame(SkyPanel panel){
		...
		f.setLayout(null);
		panel.setBounds(10,40,SkyPanel.framewidth-20,SkyPanel.frameheight-50);
		f.add(panel);
		f.pack();
		f.setResizable(false);
		f.setSize(new Dimension(SkyPanel.framewidth,SkyPanel.frameheight));
		f.setVisible(true);
		panel.requestFocus();
		return f;
	}
	...
	public void init(ImagePlus threshimp,ImagePlus skyimp,float[] objects,int[] areas,int[] arearank,findblobs3 fb,boolean showmask,float[][][] spectra,Object[] spec_stack){
		...
		if(skyimp!=null){
			...
			unmixed[0]=(float[])stack.getProcessor(2).convertToFloat().getPixelsCopy();
			unmixed[1]=(float[])stack.getProcessor(3).convertToFloat().getPixelsCopy();
			unmixed[2]=(float[])stack.getProcessor(4).convertToFloat().getPixelsCopy();
			unmixed[3]=(float[])stack.getProcessor(1).convertToFloat().getPixelsCopy();
