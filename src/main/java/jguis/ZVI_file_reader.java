 ...
 * 
 *         Modified by Jay Unruh simply for ease of use in a library
 */

public class ZVI_file_reader{
	boolean littleEndian=true;
	private int theC,theZ,numC,numZ,numT;
	// + (mb)
	// + new code
	...
	public ImagePlus get_ZVI_imp(File file){
		return get_ZVI_imp(file.getParent()+File.separator,file.getName());
	}

	/** Executes the plugin. */
	public ImagePlus get_ZVI_imp(String directory,String fileName){
		if(fileName==null)
			return null;
		...
		if(numC*numZ*numT>1){
			...
			imp.setDimensions(numC,numZ,numT);
			if(numC>1){
