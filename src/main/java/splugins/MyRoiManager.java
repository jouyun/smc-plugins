...
public class MyRoiManager extends PlugInFrame implements ActionListener, ItemListener, MouseListener, MouseWheelListener, ListSelectionListener {
...
/* Constructs and displays an MyRoiManager. */
public MyRoiManager() {
    super("ROI Manager");
    if (instance!=null) {
...
/* Constructs an MyRoiManager without displaying it. */
...
private void interpolateRois() {
    ...
    if (record())
        Recorder.record("MyRoiManager", "Interpolate ROIs");
}
...
public void itemStateChanged(ItemEvent e) {
    ...
    if (source==showAllCheckbox) {
        ...
        if (Recorder.record && recordShowAll) {
            if (showAllMode)
                    Recorder.record("MyRoiManager", "Show All");
                else
                    Recorder.record("MyRoiManager", "Show None");
        }
        recordShowAll = true;
        firstTime = false;
        return;
    ...
    if (source==labelsCheckbox) {
        ...
        if (!showAllState && !editState)
            ...
            if (Recorder.record) {
                if (editState)
                    Recorder.record("MyRoiManager", "Show All with labels");
                else if (showAllState)
                    Recorder.record("MyRoiManager", "Show All without labels");
            }
...
void recordAdd(Color color, int lineWidth) {
    if (Recorder.scriptMode())
        ...
        Recorder.recordString("MyRoiManager(\"Add\", \""+getHex(color)+"\");\n");
    else if (lineWidth>1)
        Recorder.recordString("MyRoiManager(\"Add\", \""+getHex(color)+"\", "+lineWidth+");\n");
    else
        Recorder.record("MyRoiManager", "Add");
}
...
public void add(ImagePlus imp, Roi roi, int n) {
    if (IJ.debugMode && n<3 && roi!=null) IJ.log("MyRoiManager.add: "+n+" "+roi.getName());
    if (roi==null)
        return;
...
void addAndDraw(boolean altKeyDown) {
    ...
    if (record()) Recorder.record("MyRoiManager", "Add & Draw");
}
...
boolean delete(boolean replacing) {
    ...
    if (count>1 && index.length==1 && imp!=null)
        imp.deleteRoi();
    updateShowAll();
    if (record()) Recorder.record("MyRoiManager", "Delete");
    return true;
}
...
boolean update(boolean clone) {
    ...
    if (record()) Recorder.record("MyRoiManager", "Update");
    updateShowAll();
    return true;
}
...
boolean rename(String name2) {
    ...
    if (record())
        Recorder.record("MyRoiManager", "Rename", name2);
    return true;
}
...
void open(String path) {
    ...
    if (Recorder.record && !Recorder.scriptMode())
        Recorder.record("MyRoiManager", "Open", path);
...
boolean saveMultiple(int[] indexes, String path) {
    ...
    if (Recorder.record && !IJ.isMacro())
        Recorder.record("MyRoiManager", "Save", path);
    return true;
}
...
private void listRois() {
    ...
    if (record())
        Recorder.record("MyRoiManager", "List");
}
...
boolean measure(int mode) {
    ...
    if (indexes.length>1)
        IJ.run("Select None");
    if (record()) Recorder.record("MyRoiManager", "Measure");
    return true;
}   
    ...
    if (record()) {
        if (Recorder.scriptMode()) {
            ...
            if ((nSlices==1||measureAll) && onePerSlice && !appendResults)
                Recorder.record("MyRoiManager", "Multi Measure");
            else {
                String options = "";
                if (measureAll)
                    options += " measure_all";
                ...
                if (appendResults)
                    options += " append";
                Recorder.record("MyRoiManager", "multi-measure"+options);
            }
        }
    }
    return true;
...
void multiPlot() {
    ...
    if (indexes.length>1)
        IJ.run("Select None");
    if (record()) Recorder.record("MyRoiManager", "Multi Plot");
}   
...
boolean drawOrFill(int mode) {
    ...
    if (record() && (mode==DRAW||mode==FILL))
        Recorder.record("MyRoiManager", mode==DRAW?"Draw":"Fill");
...
void setProperties(Color color, int lineWidth, Color fillColor) {
    ...
    if (record()) {
        if (fillColor!=null)
            Recorder.record("MyRoiManager", "Set Fill Color", Colors.colorToString(fillColor));
        else {
            Recorder.record("MyRoiManager", "Set Color", Colors.colorToString(color!=null?color:Color.red));
            Recorder.record("MyRoiManager", "Set Line Width", lineWidth);
        }
    }
}
...
void combine() {
    ...
    if (record()) Recorder.record("MyRoiManager", "Combine");
}
...
void and() {
    ...
    if (record()) Recorder.record("MyRoiManager", "AND");
}
...
void xor() {
    ...
    if (s1!=null) imp.setRoi(s1);
    if (record()) Recorder.record("MyRoiManager", "XOR");
}
...
void sort() {
    ...
    if (record()) Recorder.record("MyRoiManager", "Sort");
}
...
private void removePositions(int position) {
    ...
    if (record()) {
        if (removeChannels) Recorder.record("MyRoiManager", "Remove Channel Info");
        if (removeSlices) Recorder.record("MyRoiManager", "Remove Slice Info");
        if (removeFrames) Recorder.record("MyRoiManager", "Remove Frame Info");
    }
}
...
private void options() {
    ...
    if (record()) {
        Recorder.record("MyRoiManager", "Associate", Prefs.showAllSliceOnly?"true":"false");
        Recorder.record("MyRoiManager", "Centered", restoreCentered?"true":"false");
        Recorder.record("MyRoiManager", "UseNames", Prefs.useNamesAsLabels?"true":"false");
    }
}
...
void split() {
    ...
    if (record()) Recorder.record("MyRoiManager", "Split");
}
...
/** Returns a reference to the ROI Manager window or to the
    macro batch mode MyRoiManager, or null if neither exists. */
...
/** Returns the name of the ROI with the specified index.
    ...
    <pre>call("ij.plugin.frame.MyRoiManager.getName", index)</pre>
...
/** Clears this MyRoiManager so that it contains no ROIs. */
public void reset() {
...
private void translate() {
    ...
    if (record()) {
        if (Recorder.scriptMode())
            ...
            Recorder.record("MyRoiManager", "translate", (int)dx, (int)dy);
    }
}
...
public void deselect() {
    ...
    for (int i=0; i<n; i++)
        list.clearSelection();
    if (record()) Recorder.record("MyRoiManager", "Deselect");
    return;
}
...
public void valueChanged(ListSelectionEvent e) {
    ...
    if (getCount()==0) {
        if (recordInEvent())
            Recorder.record("MyRoiManager", "Deselect");
        return;
    }
    ...
    if (WindowManager.getCurrentImage()!=null) {
        ...
        if (recordInEvent()) {
            ...
            if (Recorder.scriptMode()) {
                ...
                if (selected.length == 1)
                    Recorder.recordString("MyRoiManager(\"Select\", " + arg + ");\n");
                else
                    Recorder.recordString("MyRoiManager(\"Select\", newArray(" + arg + "));\n");
            }
        }
    }
}
