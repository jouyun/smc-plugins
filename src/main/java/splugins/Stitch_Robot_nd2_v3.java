...
public class Stitch_Robot_nd2_v3 implements PlugIn {
	...
	public void run(String arg0) {
		...
		for (int p=0; p<1000; p++)
		{
			...
			IJ.run("SIMR Nd2 Reader", "imagefile=["+image_path+"]");
		...
		float pix_size=(float)pixel_scaling;
		for (int j=0; j<xy_pts[0].length; j++)
		{
			xy_pts[0][j]=(float) (xy_pts[0][j])/pix_size;
			xy_pts[1][j]=(float) (xy_pts[1][j])/pix_size;
		}
		stitch_generic.write_tile_config_alternative_name(base_dir, xy_pts[0], xy_pts[1], "Tiffs");
		...
		ImageWindow full_image=WindowManager.getCurrentWindow();
		IJ.run("Duplicate...", "title=Mig duplicate channels="+stitch_channel);
		IJ.selectWindow("Mig");
		stitch_generic.stitch_projection_img(WindowManager.getCurrentImage(), xy_pts[0], xy_pts[1], base_dir);*/
		
		//WindowManager.setCurrentWindow(full_image);
		//SaveMultipageImageSequence.save_sequence(WindowManager.getCurrentImage(), base_dir, "Img");
		
		//IJ.run("Grid/Collection stitching", "type=[Positions from file] order=[Defined by TileConfiguration] directory="+base_dir+" layout_file=out.registered.txt fusion_method=["+fusion_method+"] regression_threshold=0.30 max/avg_displacement_threshold=2.50 absolute_displacement_threshold=3.50 computation_parameters=[Save computation time (but use more RAM)] image_output=[Fuse and display]");
