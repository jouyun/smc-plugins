...
*/
public class MTrack2_smc_ implements PlugInFilter, Measurements  {
   ...
   public class courtship_class { int start_frame; int number_frames;}
   ...
   public static void setProperty (String arg1, String arg2) {
	...
	public void run(ImageProcessor ip) {
      ...
      if (!skipDialogue) {
         ...
         gd.addNumericField("Pixels between flies for courtship: ", court_rng, 2);
         gd.addNumericField("Min frames to count as courtship: ", min_frames, 0);
         gd.addNumericField("Frames to count as interaction: ", min_together, 0);
         ...
         court_rng=(float)gd.getNextNumber();
         min_frames=(int)gd.getNextNumber();
         min_together=(int)gd.getNextNumber();
	...
	public void track(ImagePlus imp, int minSize, int maxSize, float maxVelocity, String directory, String filename) {
		...
		List[] theParticles = new ArrayList[nFrames];  //theParticles is a list of positions found at every frame
		int trackCount=0;
		...
		float[][] positions=new float[nFrames][3];
		for (int i=0; i<nFrames; i++) positions[i][2]=-1.0f;
		for (int i=0; i<=(nFrames-1); i++) {
			IJ.showProgress((double)i/nFrames);
			int ctr=0;
			for (ListIterator j=theParticles[i].listIterator();j.hasNext();) 
			...
			{
				particle A, B;
				A=(particle)theParticles[i].get(0);
				B=(particle)theParticles[i].get(1);
				positions[i][2]=(float)Math.sqrt(Math.pow(A.x-B.x,2.0)+Math.pow(A.y-B.y,2.0));
			}
			if (ctr==0) positions[i][2]=10000.0f;
		}
		...
		/*  OLD WAY
		//Get the data for particle 1 (0)
		for (int i=0; i<nFrames; i++) positions[i][2]=-1.0f;
		List firstTrack=(ArrayList) theTracks.get(0);
		for (ListIterator iT=firstTrack.listIterator(); iT.hasNext();)
		{
			particle aParticle=(particle) iT.next();
			positions[aParticle.z][0]=aParticle.x;
			positions[aParticle.z][1]=aParticle.y;
			//IJ.log("x: " + aParticle.x + " y: " +aParticle.y + " z: " + aParticle.z +"\n");
		}
		
		//Now loop through all other tracks
		boolean onfirst=true;
		for (ListIterator iS=theTracks.listIterator(); iS.hasNext();)
		{
			if (onfirst)
			{
				iS.next();
			...
			List cTrack=(ArrayList)iS.next();
			for (ListIterator iT=cTrack.listIterator(); iT.hasNext();)
			{
				float x,y;
				particle aParticle=(particle) iT.next();
				positions[aParticle.z][2]=(float)Math.sqrt(Math.pow(aParticle.x-positions[aParticle.z][0],2.0)+Math.pow(aParticle.y-positions[aParticle.z][1],2.0));
				//IJ.log("t: "+aParticle.z+" distance: "+positions[aParticle.z][2]+"\n");
			}
		}*/
		...
		int ii=0;
		List <courtship_class> courtships=new ArrayList();
		while (ii<nFrames)
		{
			if (positions[ii][2]<court_rng)
			//if (positions[ii][2]<0.0f)
			{
				int idx=ii, ridx=ii, fidx=ii;
				//To make it courting only, remove these comments
				//while (positions[idx][2]<0.0f&&idx<nFrames-1) idx++;
				//if (idx==nFrames||(idx-ii<min_together)) continue;
				while (positions[ridx][2]<court_rng&&ridx>0) ridx--;
				while (positions[fidx][2]<court_rng&&fidx<nFrames-1) fidx++;
				if (fidx-ridx-2>=min_frames) 
				{
					IJ.log("Start: "+(ridx+2)+" Length: "+(fidx-ridx-2)+"\n");
					courtship_class tmp=new courtship_class();
					tmp.start_frame=ridx+2;
					tmp.number_frames=fidx-ridx-2;
					courtships.add(tmp);
					ii=fidx;
				}
			}
		...
		ImagePlus new_img;
		int frames_courting=0;
		for (ListIterator iS=courtships.listIterator(); iS.hasNext();)
		{
			courtship_class tmp=(courtship_class)iS.next();
			frames_courting+=tmp.number_frames;
			new_img=NewImage.createByteImage(""+tmp.start_frame+"_"+tmp.number_frames, dimensions[0], dimensions[1], tmp.number_frames, NewImage.FILL_BLACK);
			ImageStack new_stk=new_img.getImageStack();
			ImageStack orig_stk=imp.getImageStack();
			for (int i=0; i<tmp.number_frames; i++)
			{
				ImageProcessor orig_ip=orig_stk.getProcessor(i+tmp.start_frame);
				ImageProcessor new_ip=new_stk.getProcessor(i+1);
				byte [] orig_pix=(byte [])orig_ip.getPixels();
				byte [] new_pix=(byte [])new_ip.getPixels();
				System.arraycopy(orig_pix, 0, new_pix, 0, dimensions[0]*dimensions[1]);
			}
			new_img.show();
			new_img.updateAndDraw();
		}
		IJ.log("Fraction of time spent following:  " +(((double)frames_courting)/((double)dimensions[3])));
		if(ii>-1) return;
