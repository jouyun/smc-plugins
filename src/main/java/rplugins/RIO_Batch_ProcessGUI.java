	...
	public void processFiles(){
		...
		for(int i = 0; i<fileList.length; i++){
			if(fileList[i].toString().endsWith(".tif") || fileList[i].toString().endsWith(".tiff") || fileList[i].toString().endsWith(".jpg") || fileList[i].toString().endsWith(".gif") || fileList[i].toString().endsWith(".zvi")){
				...
				String fname = dir+"\\"+imageName+"_pos("+count+").zip";
				roim.runCommand("Save",fname);
				//roiManager("Save", "C:\\Users\\rla\\Desktop\\riotest\\single\\RoiSet.zip");
				
				IJ.log(""+count);
				Roi[] roiArray = roim.getRoisAsArray();
				for(int q=count; q>0; q--){
					Roi roi = roiArray[q-1];
					//int index = roim.getRoiIndex(roi);
					int current =q-1;
					IJ.log(""+current);
					if(!getStats(roi)){
						roim.select(q);
				...
				count = roim.getCount();
				fname = dir+"\\"+imageName+"_posneg("+count+").zip";
				roim.runCommand("Save",fname);
				roim.close();
				IJ.run("Close All");
			}
