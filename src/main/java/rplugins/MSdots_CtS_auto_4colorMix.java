...
public class MSdots_CtS_auto_4colorMix implements PlugIn {
...
///////////////End new section for loop
		...
		for(int i=0;i<ndots;i++){
			if(numR==percR && numG==percG && numB==percB && numY==percY){	break;	}
			int m = 0;
			search:
			do{
...
String title = "R_"+Rcount+"_G_"+Gcount+"_B_"+Bcount+"_Y_"+Ycount+"_numCells_"+CellsNum+"__dup_"+duplicates;
imp5.setTitle(title);

String fname = "C:\\Sim\\fillin\\"+title+".tif";
new FileSaver(imp5).saveAsTiffStack(fname);
IJ.run("MSmacroRLA sim v4col");
