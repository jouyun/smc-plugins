...
public class Bezier implements PlugIn {
	...
	public void run(String arg) {
		...
		Point p0 = new Point(p.xpoints[0], p.ypoints[0]);
		Point p1 = new Point(p.xpoints[0], p.ypoints[0]);
		Point p2 = new Point(p.xpoints[1], p.ypoints[1]);
		Point p3 = new Point(p.xpoints[2], p.ypoints[2]);
		
		
		for (int i=0; i<p.npoints; i++){
			IJ.log(i+"   X:"+p.xpoints[i]+",Y:"+p.ypoints[i]);
			////write 100 pixels in between each point
			// for(int j=1; j<=10; j++){
				// int x = p.xpoints[i]+j;
				// int y = p.ypoints[i]+j;
				// prc.set(x,y,255);
		...
		getBezierCurve(p0, p1, p2, p3);
		
	}
	
	public void getBezierCurve(Point p0, Point p1, Point p2, Point p3){
		double p0x = p0.getX();
		double p0y = p0.getY();
		double p1x = p2.getX();
		double p1y = p2.getY();
