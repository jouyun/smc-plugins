...
public class NLLSglobalfit_v2 implements NLLSfitinterface_v2{
	NLLSfitinterface_v2 fitclass;
	NLLSfit_v2 singlefit;
	public double toler;
	public int maxiter;
	int fitlength,nparams,nfits;
	int[][] linkmap;
	public Object[][] fps;
	public double lambda;

	public NLLSglobalfit_v2(NLLSfitinterface_v2 fitinterface,double toler1,int maxiter1,double lambda1){
		fitclass=fitinterface;
		toler=toler1;
		maxiter=maxiter1;
		singlefit=new NLLSfit_v2(this,toler,maxiter,lambda1);
		lambda=lambda1;
	}

	public NLLSglobalfit_v2(NLLSfitinterface_v2 fitinterface,double toler1,int maxiter1){
		fitclass=fitinterface;
		toler=toler1;
		maxiter=maxiter1;
		singlefit=new NLLSfit_v2(this,toler,maxiter);
		lambda=0.0;
	}

	public NLLSglobalfit_v2(NLLSfitinterface_v2 fitinterface,int maxiter1){
		fitclass=fitinterface;
		maxiter=maxiter1;
		toler=0.0001;
		singlefit=new NLLSfit_v2(this,toler,maxiter);
	...
	public NLLSglobalfit_v2(NLLSfitinterface_v2 fitinterface,double toler1){
		fitclass=fitinterface;
		toler=toler1;
		maxiter=50;
		singlefit=new NLLSfit_v2(this,toler,maxiter);
	...
	public NLLSglobalfit_v2(NLLSfitinterface_v2 fitinterface){
		fitclass=fitinterface;
		toler=0.0001;
		maxiter=50;
		singlefit=new NLLSfit_v2(this,toler,maxiter);
	...
	public float[][] fitdata(double[][] params,int[][] vflmatrix,String[][] formulas,String[] paramsnames,double[][][] constraints,float[][] data,float[][] weights,double[] stats,double[] c2vals,
		...
		// the vfl matrix contains 0 for vary, 1 for fix, 2 for link, and 3 or
		// functional link
	...
	public double[] fitfunc(double[] params){
		...
		double[] fit=new double[fitlength*nfits];
		for(int fitindex=0;fitindex<nfits;fitindex++){
			...
			double[] temp=fitclass.fitfunc(newparams);
			System.arraycopy(temp,0,fit,fitindex*fitlength,fitlength);
		}
		return fit;
	}
